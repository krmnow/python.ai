...
class EmailToWordCounterTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, strip_headers=True, lower_case=True, remove_punctuation=True
        ...
        self.replace_numebrs = repalce_numbers
        self.stremming = stremming
    ...
    def transform(self, X, y=None):
        ...
        for email in X:
            ...
            if self.replace_urls and url_extractor is not None:
                ...
                urls.sort(key=lambda url: len(url), revers=True)
                for url in urls:
                    text = re.sub(r'\d+(?:\.\d*(?:[eE]\d+))?', 'NUMBER', text)
            if self.remove_punctuation:
                text = re.sub(r'\W+', ' ', text, flags=re.M)
            word_counts = Counter(text.split())
            if self.stremming and stremmer is not None:
                stremmer_word_counts = Counter()
...
voca_transformer = WordCounterToVectorTransformer(vocabulary_size=10)
X_few_vecrors = vocab_transformer.fit_transform(X_few_wordcounts)
print(X_few_vecrors)

print(X_few_vecrors.toarray())

print(voca_transformer.vocabulary_)
...
X_train_transformed = preprocess_pipeline.transform(X_test)
...
print("Pelnosc: {L.2f}%".format(100*recall_score(y_test, y_pred)))
